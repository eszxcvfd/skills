...
grep -q 'Reuse & close\|worker_lifecycle\|pane close' "$SKILL_DIR/SKILL.md" && ok "SKILL reuse/close" || bad "SKILL missing reuse/close"
grep -q 'Prefer reusing' "$SKILL_DIR/SKILL.md" && ok "SKILL prefer reuse rule" || bad "SKILL missing prefer reuse"
