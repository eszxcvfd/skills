...
grep -q 'PLAN_APPROVED: true' "$SKILL_DIR/SKILL.md" && ok "approved handoff" || bad "no approved handoff"
grep -q 'never respawn an accepted or currently running task' "$SKILL_DIR/SKILL.md" && ok "handoff replay guard" || bad "no handoff replay guard"
grep -q 'RUN_ID="${RUN_KEY:-' "$SKILL_DIR/SKILL.md" && ok "stable handoff run id" || bad "no stable handoff run id"
