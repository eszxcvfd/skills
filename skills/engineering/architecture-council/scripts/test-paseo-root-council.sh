...
require_in "$skill_file" 'reduced mode' 'reduced Council mode is missing'
require_in "$skill_file" 'peer/delegated workers' 'reduced Council must use cheaper peer/delegated workers'
...
require_in "$skill_file" 'adjudicator' 'combined adjudicator role is missing'
require_in "$skill_file" 'challenge.*evidence|evidence.*challenge' 'adjudicator must preserve challenge and evidence checks'
require_in "$skill_file" 'reduced mode uses three' 'reduced Council must use exactly three bounded workers'
require_in "$skill_file" 'full mode uses three' 'full Council must use exactly three root agents'
require_in "$skill_file" 'Paseo root agents' 'full Council root-agent mode is missing'
require_in "$skill_file" 'without separate' 'separate review-agent overhead must stay removed'
require_in "$skill_file" 'challenger/verifier/judge' 'separate challenger/verifier/judge topology must stay removed'
...
printf 'PASS: architecture-council uses two proposers and one risk-based adjudicator\n'
