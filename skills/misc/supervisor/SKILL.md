...
Supervisor calls `codex-root` through Paseo using `${PASEO_CLI:-paseo}`. Every fresh human request to call, summon, start, open, create, or "gọi" root creates a fresh `codex-root`. Resume an existing root only when the human explicitly names an existing root/session id or asks to continue/reuse it. Before launch, read `<repo>/config.model` when present and verify the exact `[root]` provider/model/thinking values against the role provider catalog; the provider must be `codex-root`. Do not guess model prefixes or launch an unavailable model; report the exact catalog mismatch and safe alternatives instead. Read `ROOT_PROVIDER` from `[root].provider`, then launch with `${PASEO_CLI:-paseo} agent run --provider "$ROOT_PROVIDER" --model "$MODEL" --thinking "$THINKING" --mode full-access --label hierarchy=paseo --label role=root --cwd <repo> "<SUPERVISOR_DECISION packet>"`. The role catalog must expose `full-access`; use it for every fresh downstream role. For MCP `paseo_create_agent`, provider must be `<configured-provider>/<model>` after catalog verification, `settings.thinkingOptionId` must equal `[root].thinking`, and `settings.model` must not exist. Existing agents keep their original model/thinking; fresh work uses `config.model`, so treat stale-model sessions as reusable only when explicitly named. `codex-supervisor` never calls `codex-peer` directly; `codex-root` is the only role that allocates it.
