...
description: Use when the user explicitly requests a broad premise audit of a whole project or system and the system archetype itself may be wrong; not for ordinary architecture review or one named design concern.
---
...
For realtime multiplayer or MMO systems, first build the expected capability map
around simulation authority, session topology, interest management, state
replication, persistence, matchmaking, anti-cheat, overload, and operations. Do
not use that profile for unrelated domains.
