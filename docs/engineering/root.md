...
Root calls peer workers through Paseo with the `peer` provider via `${PASEO_CLI:-paseo}`. It inspects any candidate before sending; only a verified `peer` provider (or `role=peer` label) counts as peer. It starts one peer per bounded packet with `agent run --provider peer --label hierarchy=paseo --label role=peer --label parent=root --cwd <repo> "<WORK_PACKET>"`, or sends corrections only to an inspected peer with `agent send <peer-id> "<packet>"`.
