...
```bash
npx skills@latest add eszxcvfd/skills --skill=setup-matt-pocock-skills
```
...
`setup-matt-pocock-skills` teaches one repo how the engineering skills should behave in it — where issues live, what the triage labels are called, where the domain docs sit, and which root control docs govern architecture, runtime invariants, proof, and active execution.

It writes config, it does not hard-code behaviour. The engineering chain assumes `docs/agents/` config plus root control docs exist; this skill is the one-time bootstrap that produces them, discovered from your actual repo (`git remote`, existing labels, existing `CONTEXT.md`, existing control docs) and confirmed with you rather than guessed. It is prompt-driven — explore, present what it found, confirm, then write — not a deterministic scaffold.
...
- **Control docs** — `ARCHITECTURE.md`, `RUNTIME_CONSTITUTION.md`, and `PROCESS_AND_PROOF_POLICY.md` are created from seed templates when missing. `ACTIVE_EXECUTION_PLAN.md` is not created during setup unless active work is starting; `/implement` owns it per task.

The output is a set of files under `docs/agents/` — `issue-tracker.md`, `domain.md`, and `triage-labels.md` when `triage` is installed — root control docs, plus an `## Agent skills` block pointing to them in whichever of `CLAUDE.md` / `AGENTS.md` the repo already uses. Those files are the shared substrate the rest of the toolkit stands on.

## It's working if

- `issue-tracker.md` and `domain.md` land under `docs/agents/` (plus `triage-labels.md` when `triage` is installed), root control docs exist when missing, and an `## Agent skills` section appears in your `CLAUDE.md` or `AGENTS.md`.
