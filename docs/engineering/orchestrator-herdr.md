...
Nothing AFK starts until you approve the PLAN. Each PLAN / NEXT PLAN must **pick a real skill installed in the project** (inventory + `ask-matt` / routing, verified `SKILL.md` path, plus a one-line `match:`). Workers may be **reused** when idle and compatible (optional, preferred when safe) or **closed** with `herdr pane close` when the run will not use them again (default on FINISH). After the worker stops, the orchestrator must re-read the transcript, open every artifact, quality-gate, then show ORCH + NEXT PLAN and wait again.
