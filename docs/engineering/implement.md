...
`implement` builds work that has already been decided. You point it at a [ticket](https://www.aihero.dev/ai-coding-dictionary/ticket), a [spec](https://www.aihero.dev/ai-coding-dictionary/spec), or the plan you just agreed in the conversation, and it routes the work through the repository's canonical docs, writes the code, drives [tdd](https://aihero.dev/skills-tdd) at the seams, typechecks as it goes, runs [code-review](https://aihero.dev/skills-code-review) at the end, and commits to the current branch.
...
1. Read the smallest relevant Work Routing docs, then the ticket or spec, and work out the owners and seams.
2. Apply the current-contract hard-cut and test-discipline rules; do not carry compatibility or fallback debt into the implementation.
3. Drive [tdd](https://aihero.dev/skills-tdd) at the pre-agreed seams, one red-green slice at a time.
4. Typecheck often, run single test files as it goes, then run the full test suite once at the end.
...
## Work routing and hard cuts

Read the shared [Work Routing](https://github.com/mattpocock/skills/blob/main/skills/WORK-ROUTING.md)
reference before repository work. It defines the exact document set and is the
single source for lane selection, plan ownership, and closeout rules. Read only
the subset needed for the change; if the docs are silent or stale, record a
bounded inference or update the canonical owner before relying on a new rule.

For pre-publication products, keep one current contract and reject mismatches at
schema, protocol, metadata, package-identity, and compatibility-admission
boundaries before mutation. Do not introduce legacy/current branches, dual
read/write paths, compatibility facades, fallback interpretation, or
proof-only production seams. After a schema or protocol hard cut, audit the
diff, current tests, fixtures, validators, and generated artifacts for retired
contracts. Doc-only, small owner-neutral, and partial work does not trigger
closeout unless its governing plan requires it.
